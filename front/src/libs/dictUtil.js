...
dictUtil.initDictData = function (vm) {
    ...
    axios.get(getDictData + "medicineType").then(res => {
        if(res.success){
            vm.$store.commit("setMedicineType", res.result);
        }
    });
    axios.get(getDictData + "examineType").then(res => {
        if(res.success){
            vm.$store.commit("setExamineType", res.result);
        }
    });
};
